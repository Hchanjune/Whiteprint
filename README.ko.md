...
| `adapter:event:outbox` / `:inbox` | Outbox/Inbox 폴링 및 멱등성 처리 연결 — `adapter.event.outbox/inbox.infrastructure-implementation`으로 JPA·MongoDB 백엔드 선택 가능; MongoDB 백엔드는 웹 스택에 따라 자동 선택 — 서블릿은 `MongoTemplate`(sync), 리액티브는 `ReactiveMongoTemplate`을 `ScheduledThreadPoolExecutor` 스레드에서 `.block()`으로 사용(Netty 이벤트 루프와 완전히 분리되어 안전); `StarterConfigGuardFilter`가 JPA를 사용하지 않는 서비스에서 Spring Boot JPA/JDBC 자동 구성을 자동 제외 |
...
- **동적 Repository 스캔** — yaml에 패키지만 선언하면(`entity-packages-to-scan` / `repository-packages-to-scan`) 자동 스캔; 사용 서비스에서 `@EnableJpaRepositories`·`@EnableReactiveMongoRepositories` 작성 불필요. **Repository 등록의 주체는 스택당 하나** — `JpaRepositoryRegistrar`(`adapter:persistence:servlet` 소유)가 `org.whiteprint` 전체 패키지를 한 번에 스캔하여 모든 플랫폼 모듈과 사용 서비스의 JPA Repository를 일괄 등록하고, `MongoConfiguration`(동일 모듈)이 서블릿 스택 MongoDB Repository를, `ReactiveMongoConfiguration`(`adapter:persistence:reactive`)이 리액티브 MongoDB Repository를 각각 담당합니다. 다른 위치 — 플랫폼 모듈의 `@Configuration`, 사용 서비스의 `@SpringBootApplication`, 또는 `JpaEventOutboxConfiguration` 같은 개별 어댑터 설정 — 에서 `@EnableJpaRepositories`·`@EnableMongoRepositories`를 추가하면 Spring이 동일한 빈을 두 번 등록하여 시작 시 duplicate-bean 오류가 발생합니다
- **스케줄링** — publisher·subscriber 어댑터가 `@EnableScheduling`을 자동으로 적용하므로 사용 서비스에서 별도 선언 불필요
- **JPA 가드** — `StarterConfigGuardFilter`(`AutoConfigurationImportFilter`)가 시작 시점에 `adapter.event.outbox/inbox.infrastructure-implementation`과 `adapter.persistence.infrastructure-implementation`을 검사하여, 어느 모듈도 `jpa`를 사용하지 않으면 Spring Boot의 JPA·JDBC 자동 구성을 자동 제외 — 리액티브 전용 서비스에서 "Failed to configure a DataSource" 오류 발생 없음
- **MongoDB 가드** — `StarterConfigGuardFilter`(`AutoConfigurationImportFilter`)가 MongoDB 자동 구성(sync·reactive 전체)을 자동 제외 — MongoDB 드라이버가 전이 의존성으로 classpath에 올라오더라도 `localhost:27017`을 향하는 불필요한 연결이 생성되지 않음
- **Outbox/Inbox 듀얼 스택 MongoDB** — 서블릿 서비스는 `MongoTemplate`(sync), 리액티브 서비스는 `ReactiveMongoTemplate`을 사용 — HTTP 요청은 Netty 이벤트 루프 위에서 non-blocking으로 처리되고, Outbox/Inbox 폴링은 이벤트 루프와 완전히 분리된 `ScheduledThreadPoolExecutor` 위에서 `.block()`을 안전하게 사용; `@ConditionalOnWebApplication`으로 구현체가 자동 선택됨
