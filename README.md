...
- **JPA guard** — `StarterConfigGuardFilter` (`AutoConfigurationImportFilter`) inspects `adapter.event.outbox/inbox.infrastructure-implementation` and `adapter.persistence.infrastructure-implementation` at startup; if none resolves to `jpa`, Spring Boot's JPA and JDBC auto-configurations are excluded automatically — reactive-only services never fail with "Failed to configure a DataSource"
- **MongoDB guard** — `StarterConfigGuardFilter` (`AutoConfigurationImportFilter`) automatically excludes Spring Boot's MongoDB auto-configurations (sync and reactive) — prevents unwanted `MongoClient` connections to `localhost:27017` when the MongoDB driver is transitively on the classpath
